...
pub unsafe extern "system" fn plugin() {
    ...
    let CRTTISystemRegisterGlobalFunction: unsafe extern "thiscall" fn(*mut CRTTISystem, *mut CFunction) =
...
pub unsafe fn resolve_fn<F: FnPtr>(offset: usize) -> F {
    ...
    static BASE: LazyLock<Base> =
        LazyLock::new(|| unsafe { Base(GetModuleHandleW(PCWSTR::null()).unwrap_unchecked().0) });
